...
export function PreviewDialog({ deviceId, preview, onClose }: PreviewDialogProps) {
  const { showToast } = useToast();

  const handleDownload = async () => {
    if (!preview) return;
    try {
      const blob = await devicesService.downloadAttachment(deviceId, preview.attachment.id);
